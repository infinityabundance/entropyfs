...
- The campaign's DSFB investigation (5+5 repeated runs) found: with DSFB
  ...
  further study.
- The campaign's H2 experiment (synthetic drift corpus) is a partial
  result: temporal adjacency saves 7.2% vs shuffled history, but the
  current base+residual implementation loses to re-encoding because the
  derived chunk index retains full per-content-id descriptors — an
  identified structural cost, not a claimed win.
