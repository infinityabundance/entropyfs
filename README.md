...
- The source-corpus progression is now sealed across eras: `923df7b` pure
  ...
  **EntropyFS full 4.070× with SequenceDict** on the packed stream, and
  ...
  on a real tree of separately-written files (279/282 single-chunk),
  per-file zstd -1 is 3.541× and EntropyFS per-file writes are **2.182×,
  rising to 2.328× after the shared-dict pass** — so most of the
