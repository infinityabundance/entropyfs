...
pub enum WorkerTask {
    ...
    EncodeChunk {
        /// The owning request (generated by `SearchPool::alloc_request_id`).
        request_id: u64,
    ...
    DecodeExtent {
        ...
        limits: crate::core::limits::Limits,
        /// The caller's read window (assembly math, mirroring
        /// `materialize_into_window`).
        offset: u64,
        /// The caller's read window end.
        end: u64,
        /// The caller's output buffer length.
        avail: usize,
    },
}
...
/// The outcome of one executed task (Phase-11E).
#[derive(Debug, Clone)]
...
impl SearchPool {
    ...
    /// Submit one request's tasks (the caller stamped them with the id
    /// from [`SearchPool::alloc_request_id`]). Blocks under backpressure.
    pub fn submit(&self, request_id: u64, tasks: Vec<WorkerTask>) -> PoolSubmit {
        let rt = self.runtime.lock().expect("pool runtime poisoned");
